		...
		if (e.getSource()==registerButton) {
		  ...
		  registerButton2.setBounds(10, 80, 130, 25);
		   ...
		   if (e.getSource()==registerButton2) {
			   frame1.setVisible(false);
				...
				frame1.setVisible(true); 		
				 ...
				 adminlogin.addtouserlogins(x, y);
				adminlogin.method2(adminlogin.getmap());
		   ...
		   if (e.getSource()==resetpasswordButton) {
			 ...
			 int s=0;
			   for (String i : adminlogin.getmap().keySet()) {
				  if (i.equals(x)) {
					  s=1;
					   
				       // messageLabel.setText("Your password is " + userlogin.getmap().get(i) );
				        JOptionPane.showMessageDialog(null, "Your Password is " + adminlogin.getmap().get(i));
		   ...
		   if (e.getSource()==loginButton) {
			   ...
			   for (String i : adminlogin.getmap().keySet()) {
					  if (i.equals(x)) {
						  s=1;
						  if (y.equals(adminlogin.getmap().get(i)) ) {
							  frame.setVisible(false);
								 new leaguetable() ;
					   }
						  else {
